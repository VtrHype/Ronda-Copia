document.addEventListener('DOMContentLoaded', () => {
    ...
    addRowButton.addEventListener('click', () => {
        ...
        newRow.querySelector('.finalizar-ronda').addEventListener('click', () => {
            ...
            saveToLocalStorage(); // Salvar estado ao finalizar ronda
        });
    });
    ...
    function limparPlanilha() {
        ...
        addRowButton.disabled = false;
        localStorage.removeItem('formState'); // Limpar localStorage ao finalizar a ronda
    }
    ...
    function saveToLocalStorage() {
        const formState = {
            ...
            kmInicio: document.getElementById('kmInicio').value,
            kmFinal: document.getElementById('kmFinal').value,
            data: document.getElementById('data').value,
            rows: Array.from(document.querySelectorAll('#in tr')).map(row => ({
                cliente: row.querySelector('.cliente').value,
                hChegada: row.querySelector('.h_chegada').value,
                hSaida: row.querySelector('.h_saida').value,
                contatoLocal: row.querySelector('.contato_local').value,
                motivoDisparo: row.querySelector('.motivo_disparo').value,
    ...
    function loadFromLocalStorage() {
        ...
        if (formState) {
            document.getElementById('nome').value = formState.nome;
            document.getElementById('matricula').value = formState.matricula;
            document.getElementById('placa').value = formState.placa;
            document.getElementById('kmInicio').value = formState.kmInicio;
            document.getElementById('kmFinal').value = formState.kmFinal;
            document.getElementById('data').value = formState.data;
            formState.rows.forEach(rowData => {
                const newRow = document.createElement('tr');
                newRow.classList.add('minha_classe');
                ...
                <td>
                                <label for="cliente">Cliente</label>
                                <input type="text" class="cliente" value="${rowData.cliente}" style="text-transform: uppercase;">
                            </td>
                            
                            <td>
                                <label for="contato_local">Contato no Local</label>
                                <input type="text" class="contato_local" value="${rowData.contatoLocal}" style="text-transform: uppercase;">
                            </td>
                            <td>
                            <label for="motivo_disparo">Motivo</label>
                            <select class="motivo_disparo" value="${rowData.motivoDisparo}" style="text-transform: uppercase;">
                            <option>Selecione</option>
                            <option>Ronda</option>
                            <option>Ronda Interna</option>
                            <option>Entrega de Equipamentos</option>
                            ...
                            <td>
                                <label for="descricao">Descrição</label>
                                <input type="text" class="descricao" value="${rowData.descricao}" style="text-transform: uppercase;">
                            </td>
                            
                            <td>
                                <label for="h_chegada">Horário de Chegada</label>
                                <input type="time" class="h_chegada" value="${rowData.hChegada}" style="text-transform: uppercase; display: block;">
                            </td>
                            <td class="btn-container">
                                <label for="h_saida">Horário de Saída</label>
                                <input type="time" class="h_saida" value="${rowData.hSaida}" style="text-transform: uppercase; display: ${rowData.hSaida ? 'inline' : 'none'};">
                                <button class="btn finalizar-ronda">Finalizar Ronda</button>
                            </td>
                `;
    ...
    loadFromLocalStorage(); // Carregar o estado ao abrir o aplicativo
